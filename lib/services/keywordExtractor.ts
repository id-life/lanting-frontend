 ...
 * @returns 提取的关键词数组
 */
export const extractKeywordsFromSummary = (summary?: string | null): string[] => {
  if (!summary || summary.trim().length < 10) return []; // 摘要过短不处理

  const text = summary.toLowerCase().replace(/[.,!?()\[\]{}<>:;"'“”‘’\n\r\t]/g, ' '); // 移除标点和换行

  const words = text.split(/\s+/).filter((word) => word.length > 1 && !stopWords.has(word) && !/^\d+$/.test(word));
...
export const generateKeywordSuggestions = (
  ...
  const uniqueExtracted = extractedKeywords.filter(
    (keyword) => !predefinedMatches.some((pm) => pm.toLowerCase() === keyword.toLowerCase()),
  );
