  ...
  try {
    ...
    if (success && data) {
      ...
      const description = archive.remarks
        ? archive.remarks.substring(0, 160).replace(/\s+/g, ' ').trim() + (archive.remarks.length > 160 ? '...' : '')
        : '兰亭文存中的一篇文章。';
...
export default function ArchiveDetailLayout({ children }: { children: React.ReactNode }) {
  return <>{children}</>;
}
