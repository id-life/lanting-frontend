...
const MiscRecipes = () => {
  ...
  return (
    <ChapterCard title={<h2 className="text-xl font-medium">随园食单</h2>} defaultActiveKey="misc-recipes">
      {loading ? (
        ...
        <div className="prose prose-sm react-markdown! max-w-none">
          <ReactMarkdown
            ...
            components={{
              img: ({ alt, src, title }) => {
                if (!src) return null;
                const resolvedSrc = (src as string).startsWith('http') ? src : `${CDN_DOMAIN}/archives/origs/${src}`;
